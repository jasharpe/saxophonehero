...
bool set_contains2(std::set<int>& set, int element);
...
public:
  ...
  Matrix4x4(double *vals, bool openGL = false)
  {
    if (openGL) {
      double r[16];
      r[0] = vals[0];
  ...
  // dumps the matrix into data in OpenGL format
  void dump(double* data) const {
    data[0] = v_[0];
    data[1] = v_[4];
    data[2] = v_[8];
    data[3] = v_[12];
